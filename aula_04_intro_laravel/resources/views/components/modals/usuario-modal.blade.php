<div x-cloak {{ $attributes }} x-show="{{ $attributes['trigger'] }} == $el.id"
    x-bind:class="{{ $attributes['trigger'] }} == $el.id ?
        $el.className =
    ...
    <div class='flex flex-col w-full h-full pt-10 bg-gray-600/50' ">
        <div @click.away="idmodal=null"
        class="flex flex-col justify-center w-1/2 shadow dark:bg-gray-700 h-auto m-1 p-3 bg-white self-center rounded-md">
            <div class="p-2 mb-2 border-b-2 border-gray-300">
                <h1 class="text-2xl text-white text-center">{{$title}}</h1>
            </div>
        {{ $slot }}
        </div>
    </div>
